...
class gen_op :
	...
	def __init__ (self, pop, Pc, Pm, fitness) :
		self.pop 		= pop
		self.total 		= 0
		self.CDF			= []
		self.intgen 	= []
	...
	def pop_sum (self) :
		for agent in self.pop.strings :
			self.total += fitness(agent,self.pop.length)
		...
		for i in range(self.pop.count) :
			...
			if num <= self.Pm :
				while count < 10 and not changed:
					...
					temp.gene[first] = temp.gene[second]
					temp.gene[second] = s				
					highest = fitness(self.newpop[i],self.pop.length)
					if fitness(temp,self.pop.length) > highest :
						self.newpop[i].gene[:] = temp.gene[:]
						changed = True
					count += 1
